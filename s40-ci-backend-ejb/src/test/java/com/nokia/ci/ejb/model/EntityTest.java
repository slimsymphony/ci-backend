...
public class EntityTest {
    ...
    @Test
    public void entitiesExtendedFromBaseEntity() throws Exception {
        String packageName = "com.nokia.ci.ejb.model";
    ...
    private void handleModels(File models, ClassLoader cl, String packageName) throws Exception {
        ...
        for (String s : strList) {
            ...
            if (clazz.isAnnotationPresent(MappedSuperclass.class) || Modifier.isAbstract(clazz.getModifiers())) {
                continue;
            }
            if (isInheritedFromBaseEntity(clazz)) {
                PojoTestUtil.testGetAndSet(clazz);
            }
        }
    }

    private boolean isInheritedFromBaseEntity(Class clazz) {
