...
$(document).ready(function() {
    // Check for support HTML5 Canvas
    var testCanvas = document.createElement("canvas");
    try {
        var test = testCanvas.getContext("2d");
    }
    catch(e) {
        $('.exportJqplot').each(function() {
            $(this).hide();
        });
    }
    
    $('.exportJqplot').click(function(e) {
        e.preventDefault();
        var chart = $(this).parent().find(".jqplot-target");
        if(chart.length > 0) {
            var picture = jqplotToImg(chart);
            window.open(picture);
        } else {
            console.log("Could not find jqPlot chart");
        }
